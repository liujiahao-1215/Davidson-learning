...
contains
  ...
  function cast_to_complex(vector) result(rs)
    !> convert an array of int into double
    integer, dimension(:), intent(in) :: vector
    complex(dp), dimension(size(vector))  :: rs
    integer :: i
    do i=1,size(vector)
      rs(i) = vector(i)
    end do

  end function cast_to_complex
...
program main
  ...
  use test_utils, only: write_vector, cast_to_complex
  ...
  call generalized_eigensolver(mtx, eigenvalues_DPR, eigenvectors_DPR, lowest, "DPAR", 100, 1d-8, iter_i, 60, stx)
